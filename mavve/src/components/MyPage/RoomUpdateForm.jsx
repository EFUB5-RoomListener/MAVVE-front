...
import VisibilityDropdown from "../RoomPage/VisibilityDropdown";
import * as S from "./RoomUpdateForm.style";
...
function RoomUpdateForm({
  ...
  const { roomCode: roomCodeFromParams } = useParams();
  const roomCode = roomCodeProp || roomCodeFromParams;
  ...
  useEffect(() => {
    console.log("✅ roomInfo값:", roomInfo);
    setSelected(roomInfo.visibility ? "전체 공개" : "친구 공개");
  }, [roomInfo.visibility]);
        ...
        <S.FormBottom>
          ...
          <S.FormRight>
            ...
            <S.InputWrapper $isTitleFocused={isTitleFocused}>
              ...
              />
              <S.CharCount>
                <span className={title.length > 15 ? "over" : ""}>
                  {title.length}
                </span>
            ...
            >
              {tags.map((tag) => (
                <S.Tag key={tag} onClick={() => handleDeleteTag(tag)}>
                  #{tag}
                </S.Tag>
              ))}
